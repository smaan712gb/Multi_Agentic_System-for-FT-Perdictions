...
# Initialize the agent using a custom model
from agno.models.openai.chat import OpenAIChat

# Create a custom model for DeepSeek
...
# Override the role_map to use the correct role names for DeepSeek
deepseek_model.role_map = {
    "system": "system",
    "user": "user",
    "assistant": "assistant",
    "tool": "tool"
...
def analyze_futures(symbol: str) -> Dict[str, Any]:
    ...
    predictions_dir = os.path.join("data", "predictions", "deepseek", symbol)
    ...
    volume_profile_data = get_volume_profile(symbol, interval="5min")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python deepseek.py <symbol>")
        sys.exit(1)
